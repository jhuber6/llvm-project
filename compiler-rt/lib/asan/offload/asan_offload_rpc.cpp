...
uint32_t handleLibc(void *PortPtr, uint32_t) {
  auto &Port = *reinterpret_cast<rpc::Server::Port *>(PortPtr);
...
void drainDevice(DeviceRpc &D) {
  ...
  while (auto Port = Server.try_open(D.Lanes)) {
    uint32_t Status = handleReport(&*Port, D.Lanes);
    if (Status == rpc::RPC_UNHANDLED_OPCODE)
      Status = handleLibc(&*Port, D.Lanes);
    if (Status == rpc::RPC_UNHANDLED_OPCODE)
...
bool initDevice(uptr Ordinal, hsa_agent_t Agent) {
  ...
  D.Buffer = Buffer;
  VReport(1,
          "%s: serving device ASan reports on GPU %zu, %u ports, %u lanes\n",
...
void StartRpc(hsa_executable_t Exec) {
  ...
  for (uptr I = 0; I < Runtime.Devices.size(); ++I) {
    ...
    if (!initDevice(I, Runtime.Devices[I]) || !startThread()) {
      Report("ERROR: %s: failed to start device ASan RPC\n",
             SanitizerToolName);
      Die();
    }
    ...
    if (!Runtime.Copy(reinterpret_cast<void *>(Addr), &Client,
                      sizeof(Client))) {
      Report("ERROR: %s: failed to install device ASan RPC client\n",
             SanitizerToolName);
      Die();
    }
  }

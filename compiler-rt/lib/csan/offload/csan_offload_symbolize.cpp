...
void TrackDeviceImage(uptr LoadBase, uptr LoadSize, const void *Storage,
  ...
  if (Storage && StorageSize) {
    Img.Bytes = MmapOrDie(StorageSize, "csan device image");
...
bool SymbolizeOffloadData(uptr Addr, DataInfo *Info) {
  if (!Addr || !Info)
    return false;
  ...
  if (!Symbolizer::GetOrInit()->SymbolizeModuleData(Path, Offset, Info))
    return false;

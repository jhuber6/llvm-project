...
uint32_t handleReport(void *PortPtr, uint32_t) {
  ...
  if (Port.get_opcode() != CSAN_OFFLOAD_REPORT_OPCODE)
...
bool initDevice(uptr Ordinal, hsa_agent_t Agent) {
  ...
  D.Buffer = Buffer;
  VReport(1,
          "%s: serving device CSan reports on GPU %zu, %u ports, %u lanes\n",
...
void StartRpc(hsa_executable_t Exec) {
  ...
  for (uptr I = 0; I < Runtime.Devices.size(); ++I) {
    ...
    if (!initDevice(I, Runtime.Devices[I]) || !startThread()) {
      Report("ERROR: %s: failed to start device CSan RPC\n",
             SanitizerToolName);
      Die();
    }
    ...
    if (!Runtime.Copy(reinterpret_cast<void *>(Addr), &Client,
                      sizeof(Client))) {
      Report("ERROR: %s: failed to install device CSan RPC client\n",
             SanitizerToolName);
      Die();
    }
  }

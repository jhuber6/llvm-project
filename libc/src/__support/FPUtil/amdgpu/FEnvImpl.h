...
// exceptions.
LIBC_INLINE void set_trap_status(uint32_t status) {
  __builtin_amdgcn_s_setreg(HW_REG_TRAPSTS, status);
}

LIBC_INLINE uint32_t get_trap_status() {
  return __builtin_amdgcn_s_getreg(HW_REG_TRAPSTS);
}
...
LIBC_INLINE void set_enabled_trap(uint32_t flags) {
  __builtin_amdgcn_s_setreg(HW_REG_MODE_EXCP, flags);
}

LIBC_INLINE uint32_t get_enabled_trap() {
...
LIBC_INLINE void set_rounding_mode(uint32_t flags) {
  __builtin_amdgcn_s_setreg(HW_REG_MODE_ROUND, flags << 2 | flags);
}

LIBC_INLINE uint32_t get_rounding_mode() {
  return __builtin_amdgcn_s_getreg(HW_REG_MODE_ROUND) >> 2;
}
...
LIBC_INLINE int get_except() {
  return internal::get_except_value_for_status(internal::get_trap_status());
}

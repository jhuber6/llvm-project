...
template <bool T> struct Port {
  ...
  LIBC_INLINE uint64_t get_lane_mask() const { return lane_mask; }
...
/// multiples of the packet length. When executing on the CPU, the \p src and \p
/// size pointers must be an array of the current lane size.
template <bool T>
LIBC_INLINE void Port<T>::send_n(const void *const *src, uint64_t *size) {
  ...
  send([&](Buffer *buffer, uint32_t id) {
    ...
    num_sends = is_process_gpu()
                    ? lane_value(size, id)
                    : (lane_value(size, id) > num_sends ? lane_value(size, id)
                                                        : num_sends);
  });
...
/// multiples of the packet length. This function will allocate the memory into
/// the \p dst buffer. The size of each allocation is recorded into the \p size
/// input. If running the CPU these should be arrays of at least the lane size.
template <bool T>
template <typename A>
LIBC_INLINE void Port<T>::recv_n(void **dst, uint64_t *size, A &&alloc) {
  ...
  recv([&](Buffer *buffer, uint32_t id) {
    ...
    lane_value(dst, id) = alloc(lane_value(size, id));
    num_recvs = reinterpret_cast<uint64_t *>(buffer->data)[0];
  });

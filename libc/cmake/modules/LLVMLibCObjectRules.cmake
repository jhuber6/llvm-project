...
function(_build_gpu_object_bundle fq_target_name)
  ...
  foreach(add_gpu_obj_src ${ADD_GPU_OBJ_SRCS})
    # The packaged version will include both the NVPTX and AMDGPU builds.
    foreach(gpu_target ${AMDGPU_TARGET_TRIPLE} ${NVPTX_TARGET_TRIPLE} )
      ...
      if("${gpu_target}" STREQUAL "${NVPTX_TARGET_TRIPLE}")
        list(APPEND packager_images
             --image=file=${input_file},arch=all,triple=${NVPTX_TARGET_TRIPLE},feature=+ptx63)
      else()
        list(APPEND packager_images
             --image=file=${input_file},arch=all,triple=${AMDGPU_TARGET_TRIPLE})
       endif()
...
function(create_entrypoint_object fq_target_name)
  ...
  if(LIBC_TARGET_ARCHITECTURE_IS_GPU)
    ...
    _build_gpu_object_for_single_target(
      ...
      SRCS ${ADD_ENTRYPOINT_OBJ_SRCS}
      HDRS ${ADD_ENTRYPOINT_OBJ_HDRS}
      COMPILE_OPTIONS ${common_compile_options} -march=${LIBC_GPU_TARGET_ARCHITECTURE}
      CXX_STANDARD ${ADD_ENTRYPOINT_OBJ_CXX_STANDARD}
      DEPENDS ${full_deps_list}
      FLAGS "${ADD_ENTRYPOINT_OBJ_FLAGS}"
    )
